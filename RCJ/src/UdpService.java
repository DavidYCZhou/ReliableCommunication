...
public class UdpService {
    ...
    public static ArrayList<Packet> packBytesIntoPackets(byte[] f) throws Exception {
        ...
        while(accumulatedDataLength < bb.limit()){
            ...
            bb.get(chunk, 0, len);
            result.add(new Packet(0, len + 12, seq, chunk));
            seq ++ ;
            accumulatedDataLength += len;
        }
        // add EOT packet
        result.add(new Packet(2, 12, seq, new byte[0]));
        return result;
    }

    public void setUDPTimeout(int milisec) throws SocketException {
        udp.setSoTimeout(milisec);
    }
